...
export default function Page() {
  return (
    <DocsPage>
      ...
      <Heading num="発展">個別のトピックを深く見る</Heading>
      <p>このページで概観した内容は、次のページでそれぞれ掘り下げています。</p>
      <IndexGrid>
        <IndexCard href="/theory/algorithms/complexity" num="01" title="計算量とP対NP">
          オーダー記法の読み方、最悪・平均・償却、そして「現実的に解ける問題」の境界
        </IndexCard>
        <IndexCard href="/theory/algorithms/recursion" num="02" title="再帰と分割統治">
          ベースケースと再帰ケース、スタックの消費、半分にすると log になる理由
        </IndexCard>
        <IndexCard href="/theory/algorithms/dp" num="03" title="動的計画法と貪欲法">
          表を設計して指数を線形に落とす。その場で決める貪欲法との使い分け
        </IndexCard>
        <IndexCard href="/theory/algorithms/hash" num="04" title="ハッシュ表">
          鍵から置き場所を計算する。衝突・負荷率と、Set/Mapによる実務の最適化
        </IndexCard>
        <IndexCard href="/theory/algorithms/tree" num="05" title="木構造とヒープ">
          二分探索木・平衡木・B木・優先度付きキュー ― 階層で速さを稼ぐ
        </IndexCard>
        <IndexCard href="/theory/algorithms/graphs" num="06" title="グラフと最短経路">
          DFS・BFS・ダイクストラ法 ― つながりを扱うデータ構造
        </IndexCard>
        <IndexCard href="/theory/algorithms/string" num="07" title="文字列探索">
          KMP・ボイヤー・ムーア・ラビン・カープと、正規表現の計算量
        </IndexCard>
      </IndexGrid>
