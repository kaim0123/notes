...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p>ロックの対象範囲を<Term>ロック粒度</Term>と呼びます。テーブル全体・行・ページなど、粒度が粗いほど管理は簡単ですが他のトランザクションを長く待たせ、細かいほど並行性は上がる一方でオーバーヘッドが増えます。</p>

      <Heading num="03">分離レベル ― 独立性と並行性のトレードオフ</Heading>
      <p>独立性(Isolation)をどこまで厳密に保つかは<Term>分離レベル(隔離性水準)</Term>で調整します。厳しくするほど不整合は起きにくいですが、他のトランザクションを待たせる時間が増え、並行性(同時に処理できる量)は下がります。</p>
      <p>分離レベルを緩めると、次のような<Term>副作用(anomaly)</Term>が起こりえます。試験では「どのレベルで防げるか」の対応を問われます。</p>
      <table>
        <thead>
          <tr><th>副作用</th><th>起きること</th></tr>
        </thead>
        <tbody>
          <tr><td className="hl">ダーティリード</td><td>他トランザクションがまだ<Term>コミットしていない</Term>変更を読んでしまう。相手がロールバックすると、存在しなかったはずの値を読んだことになる</td></tr>
          <tr><td className="hl">反復不能読取り</td><td>同じ行を2回読んだとき、間に別トランザクションが更新・コミットしていて、2回目の値が1回目と違う</td></tr>
          <tr><td className="hl">ファントムリード</td><td>同じ条件で2回検索したとき、間に別トランザクションが行を追加・削除・コミットしていて、2回目の結果件数(または行の集合)が変わる</td></tr>
        </tbody>
      </table>
      <p>SQL標準で定義される代表的な4段階は次のとおりです(下ほど厳しく、並行性は下がります)。</p>
      <table>
        <thead>
          <tr><th>分離レベル</th><th>防げる副作用</th><th>備考</th></tr>
        </thead>
        <tbody>
          <tr><td className="hl">Read Uncommitted(未コミット読取り)</td><td>なし</td><td>最も緩い。ダーティリードも起こりうる。実務ではほとんど使わない</td></tr>
          <tr><td className="hl">Read Committed(コミット済読取り)</td><td>ダーティリード</td><td>多くのDBMSのデフォルトに近い。コミット済みのデータだけ読む</td></tr>
          <tr><td className="hl">Repeatable Read(反復可能読取り)</td><td>ダーティリード・反復不能読取り</td><td>同じ行を読み直しても値が変わらない。ファントムリードは起こりうる</td></tr>
          <tr><td className="hl">Serializable(直列化可能)</td><td>上記すべて(ファントムリード含む)</td><td>最も厳しい。まるでトランザクションが1本ずつ順番に実行されたかのように見える</td></tr>
        </tbody>
      </table>
      <Aside label="試験の覚え方">
      ...
      <p>ここまでの話は、原則として<Term>1つのDBMS</Term>の中でのトランザクションでした。<Term>分散データベース</Term>のように複数のDBにまたがる更新を、1つのACIDトランザクションとして扱う古典的な方式が<Term>2相コミット(Two-Phase Commit, 2PC)</Term>です。試験では「2相コミットメント」という名称で出題されます。</p>
      <table>
        <thead>
          <tr><th>フェーズ</th><th>内容</th></tr>
        </thead>
        <tbody>
          <tr><td className="hl">第1フェーズ(Prepare / 準備)</td><td><Term>コーディネータ</Term>が各参加者(各DB)に「コミットできるか」を問い合わせる。参加者はログを書いたうえで「準備OK(Yes)」か「不可(No)」を返す</td></tr>
          <tr><td className="hl">第2フェーズ(Commit / Abort)</td><td>全員がYesならCommit、1人でもNoなら全員にAbort(ロールバック)を指示する</td></tr>
        </tbody>
      </table>
      <p>2PCは「全員がOKなら一斉に確定する」という点で、単一DB内のコミット/ロールバックと考え方が近いです。一方、コーディネータや参加者がネットワーク分断で応答不能になると<Term>ブロッキング</Term>が起き、他の処理が待たされ続けるリスクがあります。</p>
      <Aside label="この先">
        マイクロサービスなど、DBが分かれた現代のシステムでは、2PCより<Link href="/database/distributed-transactions/saga">Saga</Link>や<Link href="/database/distributed-transactions/tcc">TCC</Link>のような結果整合性を前提にした方式がよく選ばれます。2PCとの違いや使い分けは「<Link href="/database/distributed-transactions">分散トランザクション</Link>」で詳しく扱います。
      </Aside>
      ...
      <CardGrid>
        ...
        <Card><CardNumber>2</CardNumber><h4>ロックと分離レベルで同時実行を調停する</h4><p>共有・専有ロックで排他し、分離レベルで独立性と並行性のバランスを取ります。</p></Card>
        <Card><CardNumber>3</CardNumber><h4>ログとバックアップで失わない</h4><p>ロールバック/ロールフォワードとバックアップの組み合わせで、障害から回復します。</p></Card>
        <Card><CardNumber>4</CardNumber><h4>2PCは複数DBの古典的な確定方式</h4><p>試験ではPrepare/Commitの2段階を押さえる。実務の分散処理はSaga/TCCへ。</p></Card>
      </CardGrid>
