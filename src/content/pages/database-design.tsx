...
export default function Page() {
  return (
    <DocsPage>
      ...
      <DiagramFrame
        slug="database-design-er-diagram"
        aspect="760 / 200"
        caption="顧客・注文・商品のER図。顧客と注文は1対多、注文と商品は主キーを「注文ID+商品ID」、属性に「数量」を持つ中間エンティティ「注文明細」を挟むことで、多対多の関係を2本の1対多に分解して表現する。"
      />
      ...
      <p>これで「マグカップの単価」は商品テーブルにただ1行だけ存在するようになり、04節の更新時異常は起きなくなりました。</p>
      ...
      <DiagramFrame
        slug="database-design-normalization-flow"
        aspect="760 / 560"
        caption="正規化の分解過程。1つの表が、2NFの適用で注文明細・注文・商品の3テーブルに、さらに3NFの適用で注文テーブルが注文・顧客・郵便番号マスタに分割され、最終的に5テーブルになる。どの分割も結合(JOIN)すれば元の1テーブルへ復元できる無損失分解になっている。"
      />
      ...
      <DocsFooter href="/database/design" />
    </DocsPage>
  );
}
