...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p>
        次の表は、アプリの代表的な機能を実装するときにDB設計で最初に確認すべき論点です。詳細は以降の節で補足します。
      </p>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <code>ORDER BY</code>対応インデックス
            </td>
          </tr>
          <tr>
            <td className="hl">ページネーション</td>
            <td>
              カーソル方式、<code>OFFSET</code>の回避
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>テナント・ユーザー単位のインデックス</td>
          </tr>
          <tr>
            <td className="hl">履歴・ログ</td>
            <td>パーティショニング、アーカイブ</td>
          </tr>
          <tr>
      ...
      <p>
        一覧画面は「条件で絞る → 順序を決める → 必要な分だけ取る」の3段階です。どの段階でDBがボトルネックになるかを意識して設計します。
      </p>
      <table>
        <thead>
          <tr>
      ...
      <DiagramFrame
        slug="database-features-composite-index"
        aspect="760 / 340"
        caption="複合インデックスの列順の比較。テナントIDとステータスを等値で絞り、更新日時で範囲指定と並び替えをするクエリでは、等値の列を先頭に、範囲・ORDER BYに使う列を最後に置くと絞り込みが最後の列まで届き、整列済みのためソート処理も省ける。先頭に範囲条件の更新日時を置くと、後続の列は絞り込みに使われない。"
      />

      <DiagramFrame
        slug="database-features-pagination"
        aspect="760 / 340"
        caption="ページネーション方式の比較。1ページ20件で501ページ目を表示するとき、OFFSET方式は先頭から10,020行を読んで10,000行を捨てるため、深いページほど遅くなる。カーソル方式は前ページ末尾の値を条件に渡してインデックスで開始位置へ直行し、20行だけを読むため何ページ目でもコストが変わらない。ただしカーソル列にもインデックスが要り、任意のページ番号への直接ジャンプはできなくなる。"
      />

      <Heading num="03">集計・ランキング ― その場計算か、先に数えるか</Heading>
      <p>
        「人気順」「売上合計」「今月の件数」は、毎回全行を数え直すと重くなります。リアルタイム性と更新コストのトレードオフで、保持方法を選びます。
      </p>
      <table>
        <thead>
          <tr>
      ...
      <p>
        時系列で並ぶデータは「新しいものからN件」というアクセスパターンが支配的です。JOINの回数と、インデックスの向きを機能に合わせて決めます。
      </p>
      <table>
        <thead>
          <tr>
      ...
      <p>
        画面に直接現れにくい機能ほど、後から手を入れるコストが大きいです。マルチテナントや監査要件があるサービスでは、最初からDBの枠組みに組み込んでおきます。
      </p>
      <table>
        <thead>
          <tr>
      ...
      <CardGrid>
        <Card>
          ...
          <h4>一覧系はインデックスの列順</h4>
          <p>フィルタ・並び替え・ページネーションは、複合インデックスとカーソル方式でまとめて設計します。</p>
        </Card>
        <Card>
          <CardNumber>2</CardNumber>
          <h4>集計系は先に数える</h4>
          <p>ランキングやダッシュボードは、カウンタ列・集計テーブル・キャッシュで読み取りを速くします。</p>
        </Card>
        <Card>
          <CardNumber>3</CardNumber>
          <h4>時系列は読み取りパターン優先</h4>
          <p>タイムライン・チャット・通知は、JOINを減らし、時系列インデックスとアーカイブを用意します。</p>
        </Card>
        <Card>
          <CardNumber>4</CardNumber>
          <h4>整合性は制約とトランザクション</h4>
          <p>いいね・フォローはユニーク制約、削除は論理削除と外部キー、更新競合はトランザクションとロックで守ります。</p>
        </Card>
      </CardGrid>

      <DocsFooter href="/database/features" />
    </DocsPage>
  );
}
