...
export default function Page() {
  return (
    <DocsPage>
      ...
      <DiagramFrame
        slug="computer-history-turing-machine"
        aspect="640 / 190"
        caption="チューリングマシンの4つの部品。無限に長いテープにマス目ごとに0や1などの記号が並び、読み書きヘッドが今いる位置の記号を読み書きする。右側には内部状態(今どのモードか)と、この状態でこの記号を読んだら何を書いて左右どちらに動くかを定めた状態表が置かれ、この4つだけであらゆる計算を表現できる。"
      />
      ...
      <DiagramFrame
        slug="computer-history-von-neumann"
        aspect="640 / 260"
        caption="ノイマン型アーキテクチャの5要素。中央に制御装置と演算装置(ALU)が縦に並び、左に入出力装置、右にメモリがつながる。メモリにはプログラムもデータも同じように置かれる。すべての装置はバスという共通の通り道でつながり、フェッチ→デコード→実行→次の命令へというサイクルを繰り返す。"
      />
      ...
      <DocsFooter href="/computer/history" />
    </DocsPage>
  );
}
