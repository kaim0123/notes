...
<div className="text-left">
          <pre className="overflow-x-auto rounded-xl border border-border bg-card p-4 px-[18px] text-[0.8rem] leading-relaxed">
            <code>{`┌───┬───┬───┬───┬───┐   ┌───┬───┬───┬───┬───┐
│ Q │ W │ E │ R │ T │   │ Y │ U │ I │ O │ P │
├───┼───┼───┼───┼───┤   ├───┼───┼───┼───┼───┤
│ A │ S │ D │ F │ G │   │ H │ J │ K │ L │ ; │
...
└───┴───┴───┴───┴───┘   └───┴───┴───┴───┴───┘
        ...
        <figcaption className="mt-2 text-[0.85rem] leading-relaxed text-muted-foreground">ベースレイヤー(QWERTY・36キー例)。ホーム段の下段表記が長押し時の修飾キー、親指は入口キー</figcaption>
      </figure>
      <p>
...
└───┴───┴───┴───┴───┘   └───┴───┴───┴───┴───┘
        ...
        <figcaption className="mt-2 text-[0.85rem] leading-relaxed text-muted-foreground">記号レイヤー例。右親指(かな)長押し中に左手で入力。括弧はペアで隣接させると打ちやすい</figcaption>
      </figure>
      <p>
...
└───┴───┴───┴───┴───┘   └───┴───┴───┴───┴───┘
  英数長押し → │保持│  押しながら右手で数字`}</code>
        ...
        <figcaption className="mt-2 text-[0.85rem] leading-relaxed text-muted-foreground">数字レイヤー例。左親指(英数)長押し中に右手でテンキー入力</figcaption>
      </figure>
      <p>
...
└───┴───┴───┴───┴───┘   └────┴────┴────┴────┴────┘
  Space長押し → │保持│`}</code>
        ...
        <figcaption className="mt-2 text-[0.85rem] leading-relaxed text-muted-foreground">ナビレイヤー例。左親指(Space)長押し中に右手でカーソル操作。HJKLが矢印になる</figcaption>
      </figure>
      <p>
      ...
      <p>
        40%キーボードは「キーを覚える」より「レイヤーという面を切り替える」感覚に慣れるかどうかがすべてです。まずはIMEの2キーと記号・数字・ナビの3レイヤーだけを作り、誤爆対策を詰めながらホームロウモッドを足していく ― この順序なら、日本語も英語も記号も、ホームポジションから一歩も動かずに打てる環境にたどり着けます。キーマップは開発環境の一部として、シェルやエディタの操作と合わせて育てていくのがおすすめです。
      </p>

      <DocsFooter href="/computer/io-devices-keyboard" />
    </DocsPage>
  );
}
