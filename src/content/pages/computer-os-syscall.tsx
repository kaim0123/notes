...
export default function Page() {
  return (
    <DocsPage>
      ...
      <p>前のページ「<Link href="/computer/os-kernel">カーネルの役割と設計</Link>」で見たとおり、ハードウェアを直接触れるのはカーネルだけです。では、ユーザーモードで動く普通のアプリは、どうやってその力を借りるのでしょうか。その正式な手段が<Term>システムコール</Term>です。</p>
      ...
      <DiagramFrame
        slug="computer-os-syscall-mode-switch"
        aspect="620 / 200"
        caption="システムコールでのモード切り替え。上のユーザーモード(一般客エリア)でアプリケーションが動き、下のカーネルモード(スタッフ専用)にカーネルがいる。①アプリがカーネルへ依頼を出し、②カーネルが処理して結果を返し、ユーザーモードへ復帰する。"
      />
      ...
      <h3>プロセス制御の要点</h3>
      <p><code>fork</code>は親プロセスから子プロセスを複製する「分身の術」で、戻り値で親子を区別します ― 親には子のPID、子には<code>0</code>が返ります。<code>exec</code>は別のプログラムに変身し、<code>exit</code>で終了、<code>wait</code>で子の終了を待ちます。詳しくは「<Link href="/computer/os-process">プロセスとスレッド</Link>」で扱います。</p>
      ...
      <DocsFooter href="/computer/os-syscall" />
    </DocsPage>
  );
}
