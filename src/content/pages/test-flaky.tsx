...
await expect(page.locator(".status")).toHaveText("保存しました");`}</code>
      ...
      <Heading num="04">待ち方が正しくても、揺れるとき</Heading>
      <p>
        待ち方をすべて状態待ちに直しても、なお残るものがあります。原因が<strong>テストの書き方ではなく、実行のさせ方の側にある</strong>場合です。実際に報告されている典型例が、ホバーとツールチップのテストです ― 単独では必ず通るのに、並列実行にした途端に不安定になります。
      </p>
      <p>
        原因は待ち漏れではありません。複数のテストが同時に走るとブラウザに負荷が集中し、<strong>性能を保つためにブラウザ自身が挙動を変え始めます</strong>。連続したマウス移動のイベントが1つに統合され、描画の処理が滞り、フレームが飛ぶ。結果としてツールチップが出る時刻そのものが予測できなくなります。
      </p>
      <p>
        ページの読み込みと違い、ホバーは<Term>小さく優先度の低いイベントの連鎖</Term>です。負荷がかかったとき、真っ先に間引かれる側にあります。
      </p>

      <Aside label="待ち時間を伸ばしても直らない">
        この種の不安定さは、<strong>待つ時間を増やしても再現性のある解決になりません</strong>。表示される時刻が遅いのではなく、揺れているからです。対処は実行モデル側 ― 該当するテスト群だけを直列で走らせ、ブラウザのイベント処理を過負荷にしないことです。
      </Aside>

      <p>
        ここから引き出せる教訓は、<Link href="/test/e2e-waiting">固定スリープの禁止</Link>と表裏一体です ― <strong>固定時間の待機で誤魔化せる不安定さはそもそも少なく、多くは実行のさせ方そのものを見直す必要があります</strong>。原因を8つに分類する意味は、ここにあります。分類を飛ばして待ち時間を足すと、確率が下がっただけの状態を「直した」と誤認します。
      </p>

      <Heading num="05">見つける仕組みと、直したと言い切る手順</Heading>
      <p>
      ...
      <Heading num="06">自動再実行の是非</Heading>
      <p>
      ...
      <Heading num="07">そもそも減らす</Heading>
      <p>
